...
function matchScore(queryText: string, article: ArticleWithType, extraKeywords?: string[]): number {
  ...
  // Exact phrase match in title (stripped of "Scrap Mechanic")
  const strippedQuery = normalizedQuery.replace(/scrap mechanics?\s*/g, '').trim()
  const strippedTitle = normalizedTitle.replace(/scrap mechanics?\s*/g, '').trim()
